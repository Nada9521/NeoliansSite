package com.neolians.common.utils.cssselectortoxpath.utilities.basetestcases;
...
import com.neolians.common.utils.cssselectortoxpath.utilities.CssSelectorStringSplitter;
import com.neolians.common.utils.cssselectortoxpath.utilities.CssSelectorToXPathConverterUnsupportedPseudoClassException;
...
public class BaseCssSelectorToXpathTestCase {
	...
	public static List<BaseCssSelectorToXpathTestCase> getBaseCssSelectorToXpathTestCases(boolean includeOrCase) {
		final List<BaseCssSelectorToXpathTestCase> baseCases = new ArrayList<>();
		// Simple selectors
		...
		addBaseCaseToXPath(baseCases, "classSelector", "form.classSelector",
				"//form[contains(concat(\" \",normalize-space(@class),\" \"),\" classSelector \")]");
		...
		addBaseCaseToXPath(baseCases, "moneySignEqualAttribute", "a[href$=\"pdf\"]",
				"//a[substring(@href,string-length(@href)-string-length(\"pdf\")+1)=\"pdf\"]");
		addBaseCaseToXPath(baseCases, "tildaEqualAttribute", "a[alt~=\"art\"]",
				"//a[contains(concat(\" \",normalize-space(@alt),\" \"),\" art \")]");
		addBaseCaseToXPath(baseCases, "pipeEqualAttribute", "li[data-years|=\"1900\"]",
				"//li[starts-with(@data-years,concat(\"1900\",\"-\")) or @data-years=\"1900\"]");
		// moving this test so it it does not immediately follow similar test without
		// quotes because in selenium test we did not want to put a sleep to prevent
		// stale element from occuring
		addBaseCaseToXPath(baseCases, "equalAttribute", "a[href=\"https://css-selector-to-xpath.appspot.com\"]",
				"//a[@href=\"https://css-selector-to-xpath.appspot.com\"]");
		...
		addBaseCaseToXPath(baseCases, "adjacentSiblingCombinator", "div+span",
		...
		addBaseCaseToXPath(baseCases, "only-child", "form:only-child",
				"//form[count(preceding-sibling::*)=0][count(following-sibling::*)=0]");
		...
		addBaseCaseToXPath(baseCases, "only-of-type", "span:only-of-type",
				"//span[count(preceding-sibling::span)=0][count(following-sibling::span)=0]");

		return baseCases;
	}

	private static void addBaseCaseToXPath(List<BaseCssSelectorToXpathTestCase> baseCases, String name,
			String cssSelector, String expectedXpath) {
	...
	public static Map<String, String> getBaseCssSelectorToXpathExceptionTestCases() {
		final HashMap<String, String> baseCases = new HashMap<>();
		...
		final String[] pseudoClasses = getUnimplementedPseudoClasses();
		for (final String pseudoClass : pseudoClasses) {
			baseCases.put(pseudoClass, CssSelectorToXPathConverterUnsupportedPseudoClassException
					.getPseudoClassUnsupportedError(pseudoClass));
	...
	public static String[] getUnimplementedPseudoClasses() {
		// listing every pseudo class in:
		// https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Selectors
		// so that in the future there will be some we can't handle

		return new String[] { ":active", ":active", ":any-link", ":checked", ":default", ":defined", ":dir(A)",
				":disabled",
//implemented	":empty",
				":enabled", ":first",
//implemented	":first-child",
//implemented	":first-of-type",
				":fullscreen", ":focus", ":focus-visible", ":focus-within", ":host", ":host(A)", ":host-context(A)",
				":hover", ":indeterminate", ":in-range", ":invalid", ":lang(A)",
//implemented	":last-child",
//implemented	":last-of-type",
				":left", ":link", ":not(A)", ":nth-child(A)", ":nth-last-child(A)", ":nth-last-of-type(A)",
				":nth-of-type(A)",
